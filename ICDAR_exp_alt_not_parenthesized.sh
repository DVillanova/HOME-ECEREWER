...
######################################################
mkdir not_parenthesized
python $ScriptsDir/alt_parser_parenthesized_to_continuous.py index.words-german ./not_parenthesized/index.words-german
python $ScriptsDir/alt_parser_parenthesized_to_continuous.py index.words-czech ./not_parenthesized/index.words-czech
python $ScriptsDir/alt_parser_parenthesized_to_continuous.py index.words-latin ./not_parenthesized/index.words-latin
python $ScriptsDir/alt_parser_parenthesized_to_continuous.py index.words ./not_parenthesized/index.words
...
echo "==============" >> $WorkDir/ALT_NOT_PARENTHESIZED_res_exp.txt
echo "Resultados tras añadir LM" >> $WorkDir/ALT_NOT_PARENTHESIZED_res_exp.txt
...
if $(which compute-wer &> /dev/null); then
  ...
  python $WorkDir/scripts/fix_separate_symb_output.py $TmpDir/decode/lattices/SeparateSimbols-wordtest.txt $TmpDir/decode/lattices/fixed_SeparateSymbols-wordtest.txt
  python $WorkDir/scripts/fix_separate_symb_output.py $TmpDir/decode/lattices/SeparateSimbols-word.txt $TmpDir/decode/lattices/fixed_SeparateSymbols-word.txt
...
rm -rf NER-ALT_CONTINUOUS-TEST
mkdir NER-ALT_CONTINUOUS-TEST
cd NER-ALT_CONTINUOUS-TEST
...
rm -rf NER-ALT_CONTINUOUS-GT
mkdir NER-ALT_CONTINUOUS-GT
cd NER-ALT_CONTINUOUS-GT
python ${ScriptsDir}/extract_ner_not_parenthesized.py $TmpDir/fixed_SeparateSimbols-GT_test.txt .
for f in *; do sed 's/^ //g' -i $f; done

cd ..

python ${ScriptsDir}/calc_prec_rec.py ./NER-ALT_CONTINUOUS-GT ./NER-ALT_CONTINUOUS-TEST >> $WorkDir/ALT_NOT_PARENTHESIZED_res_exp.txt
python ${ScriptsDir}/alt_dist_edicion_custom_saturated.py ./NER-ALT_CONTINUOUS-GT ./NER-ALT_CONTINUOUS-TEST >> $WorkDir/ALT_NOT_PARENTHESIZED_res_exp.txt
