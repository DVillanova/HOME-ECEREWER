...
##################################################################################################################################################################################
# Prepare files without suffixes
# rm ${PartDir}/"test".lst
...
# PREPARE DIRECTORY "ALL_LINES" IN DATA/ALL_LINES
# [ -d ${DataDir}/ALL_LINES ] ||
# {
...
#CREATION OF NDEX.WORDS (FILES WITH TRANSCRIPTION + NE)        
...
#GENERATE TRANSCRIPTIONS AT CHAR LEVEL
...
#Find folders which contain line level images
img_dirs=$(find ${DataDir}/*_charters -mindepth 4 -maxdepth 4 -type d)
#img_dirs=${DataDir}/*_charters/*/*/page/ #Alternative

#Training optical model
...
#DECODE TEST AND VAL (CER)
#TO GET CHAR LEVEL OUTPUTS => SPACE = "<SPACE>", JOIN_STR = " "
...
# Get word-level transcript hypotheses (FOR WER COMPUTATION)
awk '{
  printf("%s ", $1);
  for (i=2;i<=NF;++i) {
    if ($i == "<space>")
...
if $(which compute-wer &> /dev/null); then
  ...
  #WER with special character separation (hola! -> hola !)
...
$WorkDir/scripts/extractNER-GT.sh ${TextDir}index.words
...
# Obtaining confMats for LM integration
cd ${TmpDir}/decode
...
mkdir $WorkDir/PREC-REC/NER-DECODE
cd $WorkDir/PREC-REC/NER-DECODE
$WorkDir/scripts/extractNERHip.sh $TmpDir/decode/lattices/word-lm/hyp_word-test.txt .
...
$WorkDir/scripts/extractNER-GT.sh $TmpDir/test.txt .
for f in *; do sed 's/^ //g' -i $f; done

cd ..

python ${ScriptsDir}/calc_prec_rec.py ./NER-GT ./NER-DECODE >> $WorkDir/results-1best-decoding.txt
