...
cd ..
rm -rf $WorkDir/PREC-REC/COMBINED_NER-DECODE
mkdir $WorkDir/PREC-REC/COMBINED_NER-DECODE
cd $WorkDir/PREC-REC/COMBINED_NER-DECODE
$WorkDir/scripts/extractNERHip2.sh $TmpDir/decode/lattices/word-lm/combined_hyp_word-test.txt .
for f in *; do sed 's/^ //g' -i $f; done

cd ..

rm -rf $WorkDir/PREC-REC/ALIGNED_NER-DECODE
mkdir $WorkDir/PREC-REC/ALIGNED_NER-DECODE
cd $WorkDir/PREC-REC/ALIGNED_NER-DECODE
$WorkDir/scripts/extractNERHip2.sh $TmpDir/decode/lattices/word-lm/aligned_hyp_word-test.txt .
for f in *; do sed 's/^ //g' -i $f; done
...
python $WorkDir/scripts/dist_edicion_custom_saturated.py ./NER-GT ./COMBINED_NER-DECODE >> $WorkDir/res_exp.txt

python $WorkDir/scripts/calc_prec_rec.py ./NER-GT ./ALIGNED_NER-DECODE >> $WorkDir/res_exp.txt
python $WorkDir/scripts/dist_edicion_custom_saturated.py ./NER-GT ./ALIGNED_NER-DECODE >> $WorkDir/res_exp.txt
